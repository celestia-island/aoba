...
pub async fn test_tui_slave_with_cli_master_continuous(port1: &str, port2: &str) -> Result<()> {
    ...
    for round in 1..=ROUNDS {
        ...
        // Verify communication occurred by checking log count increased
        let actions = vec![CursorAction::CheckStatus {
            description: format!("Log count should be > 0 after round {}", round),
            path: "ports[0].log_count".to_string(),
            expected: json!(round), // Expect at least 'round' number of logs
            timeout_secs: Some(5),
            retry_interval_ms: Some(500),
        }];

        // Note: This is a simplified check - in real test we'd verify actual data
