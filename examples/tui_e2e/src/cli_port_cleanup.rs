...
pub async fn test_cli_port_release() -> Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        log::info!(
            "✅ First CLI process (PID {}) is running and has opened the port",
            pid1
        );

        // Verify the port is being held by our CLI process
        let lsof_before = Command::new("lsof").arg(&ports.port1_name).output()?;

        if lsof_before.status.success() {
            ...
            log::warn!(
                "⚠️ Unable to run lsof: {}",
                String::from_utf8_lossy(&lsof_before.stderr)
            );
            log::info!("✅ Verified: CLI process {} is holding the port", pid1);
        }
        ...
        let lsof_after = Command::new("lsof").arg(&ports.port1_name).output()?;

        if lsof_after.status.success() {
            ...
            if lsof_after_str.contains(&pid1.to_string()) {
                ...
                if lsof_after_str.trim().is_empty() {
                    log::info!("✅ Port is completely free (lsof reported no holders)");
                } else {
            ...
            log::warn!(
                "⚠️ Unable to confirm port release via lsof: {}",
                String::from_utf8_lossy(&lsof_after.stderr)
            );
        }
