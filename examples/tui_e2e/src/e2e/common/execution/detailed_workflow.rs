...
pub async fn run_detailed_multi_master_test(
    ...
    log::info!("🧪 Running detailed multi-station Master test with {} stations", configs.len());
    ...
    screenshot_ctx.capture_or_verify(&mut session, &mut cap, state.clone(), "modbus_dashboard_init").await?;
    ...
    log::warn!("    Full implementation needs screenshot_ctx threaded through all helper functions");
