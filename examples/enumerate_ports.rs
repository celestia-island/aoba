...
fn main() {
    ...
    let has_vcom1 = enriched_ports
        .iter()
        .any(|(p, _)| p.port_name.contains("vcom1"));
    let has_vcom2 = enriched_ports
        .iter()
        .any(|(p, _)| p.port_name.contains("vcom2"));
    ...
    let ttys_count = enriched_ports
        .iter()
        .filter(|(p, _)| p.port_name.contains("/ttyS") && !p.port_name.contains("vcom"))
        .count();
    let tty_other_count = enriched_ports
        .iter()
        .filter(|(p, _)| {
            p.port_name.contains("/tty")
                && !p.port_name.contains("USB")
                && !p.port_name.contains("ACM")
                && !p.port_name.contains("vcom")
    ...
    if has_vcom1 && has_vcom2 {
        ...
        let vcom1_pos = enriched_ports
            .iter()
            .position(|(p, _)| p.port_name.contains("vcom1"));
        let vcom2_pos = enriched_ports
            .iter()
            .position(|(p, _)| p.port_name.contains("vcom2"));
