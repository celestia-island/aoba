...
pub async fn test_modbus_master_slave_communication() -> Result<()> {
    ...
    if slave_screen.contains("CAFE")
        || slave_screen.contains("0xCAFE")
        || slave_screen.contains("cafe")
    {
        ...
        log::warn!(
            "This is expected on first run - the test will help identify what needs to be fixed"
        );
    }
