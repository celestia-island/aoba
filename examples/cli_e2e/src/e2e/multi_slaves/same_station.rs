...
pub async fn test_multi_slaves_same_station() -> Result<()> {
    ...
    match process.try_wait()? {
        None => {
            ...
            log::info!("✅ Multi-slaves with same station configuration process started successfully and is running");
        }
        Some(status) => {
    ...
    process.kill()?;
    process.wait()?;
    log::info!("✅ Stopped multi-slaves with same station configuration process");
