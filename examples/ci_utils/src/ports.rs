...
/// Build platform-appropriate Regex matchers for the two virtual ports.
/// Uses environment variables AOBATEST_PORT1 and AOBATEST_PORT2 if set, otherwise uses defaults.
/// DEPRECATED: Use vcom_matchers_with_ports() instead and pass ports explicitly.
...
/// Decide whether virtual serial port (vcom) tests should run on this platform.
/// Uses environment variables AOBATEST_PORT1 and AOBATEST_PORT2 if set, otherwise uses defaults.
/// DEPRECATED: Use should_run_vcom_tests_with_ports() instead and pass ports explicitly.
