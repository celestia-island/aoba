...
async fn test_navigation_and_scrolling(rows: u16, cols: u16, test_name: &str) -> Result<()> {
    ...
    let initial = cap.capture(&mut session, &format!("{test_name} - Initial state"))?;

    // Verify initial screen has port list elements
    assert!(
        initial.contains("COM")
            || initial.contains("Ports")
            || initial.contains("tty")
            || initial.contains("AOBA"),
...
mod tests {
    ...
    async fn test_port_list_scrolling_integration() {
        ...
        test_port_list_scrolling()
            .await
            .expect("Port list scrolling test failed");
    }
}
