    ...
    loop {
        match runtime.evt_rx.try_recv() {
            Ok(event) => {
                ...
                match event {
                    crate::runtime::RuntimeEvent::FrameReceived(frame) => {
                        let hex_frame = frame
                            .iter()
                            .map(|b| format!("{b:02x}"))
                            .collect::<Vec<_>>()
                ...
                .unwrap_or(false);
                        ...
                        if let Ok(response) = generate_modbus_master_response(&frame, port_arc) {
                            ...
                            if let Err(err) = runtime
                                .cmd_tx
                                .send(crate::runtime::RuntimeCommand::Write(response.clone()))
                            {
                                let warn_msg = format!(
                    ...
                    crate::runtime::RuntimeEvent::Error(error) => {
...
pub fn handle_master_query_mode(
    ...
    if let Some((request_bytes, was_pending_write)) = send_plan {
        ...
        if let Err(err) = runtime
            .cmd_tx
            .send(crate::runtime::RuntimeCommand::Write(send_payload))
        {
            let warn_msg = format!(
