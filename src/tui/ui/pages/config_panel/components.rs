...
fn create_line(
    ...
    // Create spans will be produced by the shared renderer. We collect the
    // value spans in a closure and then call `render_kv_line` below.
    let value_closure = |_state: TextState| -> Result<Vec<Span<'static>>> {
        ...
        // position, return an empty line equivalent: a single empty span.
        if label.is_empty() {
    ...
    // Map selection/editing to a TextState for indicator coloring
    let buffer = read_status(|s| Ok(s.temporarily.input_raw_buffer.clone()))
        .unwrap_or(types::ui::InputRawBuffer::None);
