...
/// Generate separate label and value vectors for the two-column config panel layout.
/// Returns a tuple of (labels, values) where each vector contains styled lines.
///
...
///
/// Labels and values use StyledSpanKind with appropriate states for highlighting/editing.
pub fn render_kv_data() -> Result<(Vec<Line<'static>>, Vec<Line<'static>>)> {
    ...
    crate::protocol::status::read_status(|app| {
        ...
        render_group1_data(&mut labels, &mut values, port_data, current_selection)?;

        // Empty line between groups
        labels.push(Line::from(vec![Span::raw("")]));
        values.push(Line::from(vec![Span::raw("")]));

        // GROUP 2: Serial parameters
        render_group2_data(&mut labels, &mut values, port_data, current_selection)?;
...
/// Render Group 1 data: Enable Port toggle + Communication Protocol selector
fn render_group1_data(
    ...
    // Create enable state for styling
    ...
    labels.push(create_label_line(&enable_label)?);
    values.push(create_value_line(&enable_value, enable_state, false)?);
    ...
    labels.push(create_label_line(&protocol_label)?);
    values.push(create_value_line(&protocol_value, protocol_state, true)?);

    Ok(())
}

/// Render Group 2 data: Serial port basic parameters
fn render_group2_data(
    labels: &mut Vec<Line<'static>>,
    values: &mut Vec<Line<'static>>,
    port_data: Option<&crate::protocol::status::types::port::PortData>,
    current_selection: usize,
    ...
    for (idx, label) in serial_labels.iter().enumerate() {
        ...
        labels.push(create_label_line(label)?);
        values.push(create_value_line(&value, state, false)?);
    }

    Ok(())
}

/// Create a styled label line
fn create_label_line(label: &str) -> Result<Line<'static>> {
    let label_span = Span::styled(
        label.to_string(),
        Style::default().add_modifier(Modifier::BOLD),
    );
    Ok(Line::from(vec![label_span]))
}

/// Create a styled value line
fn create_value_line(
    value: &str,
