...
pub fn handle_input(key: KeyEvent, bus: &Bus) -> Result<()> {
    ...
    let editing = read_status(|status| {
        Ok(!matches!(status.temporarily.input_raw_buffer, types::ui::InputRawBuffer::None))
    })?;
...
fn handle_navigation_input(key: KeyEvent, bus: &Bus) -> Result<()> {
    ...
    match key.code {
        KeyCode::Up | KeyCode::Char('k') => {
            // Move cursor up
            write_status(|status| {
                if let types::Page::ModbusDashboard { cursor, view_offset, .. } = &mut status.page {
...
fn start_text_editing(cursor: types::cursor::ModbusDashboardCursor) -> Result<()> {
    ...
    write_status(|status| {
        status.temporarily.input_raw_buffer = types::ui::InputRawBuffer::String {
            bytes: current_value.into_bytes(),
            offset: 0,
        };
        Ok(())
    })
            ...
            if let types::port::PortConfig::Modbus { masters, slaves } = &port_guard.config {
                match cursor {
                    types::cursor::ModbusDashboardCursor::StationId { index } => {
                        let item = masters.get(index).or_else(|| slaves.get(index - masters.len()));
                        return Ok(item.map(|i| i.station_id.to_string()).unwrap_or_else(|| "1".to_string()));
                    }
                    types::cursor::ModbusDashboardCursor::RegisterStartAddress { index } => {
                        let item = masters.get(index).or_else(|| slaves.get(index - masters.len()));
                        return Ok(item.map(|i| i.register_address.to_string()).unwrap_or_else(|| "0".to_string()));
                    }
                    types::cursor::ModbusDashboardCursor::RegisterLength { index } => {
                        let item = masters.get(index).or_else(|| slaves.get(index - masters.len()));
                        return Ok(item.map(|i| i.register_length.to_string()).unwrap_or_else(|| "1".to_string()));
                    }
                    types::cursor::ModbusDashboardCursor::Register { slave_index, register_index } => {
                        let item = masters.get(slave_index).or_else(|| slaves.get(slave_index - masters.len()));
