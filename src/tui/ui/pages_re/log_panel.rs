...
pub fn render(f: &mut Frame, area: Rect, app: &Status) {
    ...
    // Bottom text input for sending
    render_log_input(f, chunks[1], app);
}

pub fn page_bottom_hints(app: &Status) -> Vec<String> {
    let mut hints: Vec<String> = Vec::new();
    hints.push(lang().hotkeys.hint_move_vertical.as_str().to_string());
    hints.push(lang().hotkeys.hint_log_follow_toggle.as_str().to_string());
    hints.push(lang().hotkeys.hint_log_clear.as_str().to_string());
