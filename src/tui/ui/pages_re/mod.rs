...
pub fn bottom_hints_for_app(app: &Status) -> Vec<String> {
    if app.page.subpage_active {
        ...
        match app.page.subpage_tab_index {
            SubpageTab::Config => {
                return config_panel::page_bottom_hints(app)
            }
            SubpageTab::Body => {
                // Body may be Modbus or MQTT depending on app mode
                match app.page.app_mode {
                    crate::protocol::status::AppMode::Modbus => {
                        return modbus_panel::page_bottom_hints(app)
                    }
                    crate::protocol::status::AppMode::Mqtt => {
                        return mqtt_panel::page_bottom_hints(app)
                    }
                }
            }
            SubpageTab::Log => return log_panel::page_bottom_hints(app),
        }
    }
...
pub fn map_key_in_page(key: KeyEvent, app: &Status) -> Option<Action> {
    if app.page.subpage_active {
        use crate::protocol::status::SubpageTab;
        match app.page.subpage_tab_index {
            SubpageTab::Config => return config_panel::map_key(key, app),
            SubpageTab::Body => match app.page.app_mode {
                crate::protocol::status::AppMode::Modbus => {
                    return modbus_panel::map_key(key, app)
                }
                crate::protocol::status::AppMode::Mqtt => {
                    return mqtt_panel::map_key(key, app)
                }
            },
            SubpageTab::Log => return log_panel::map_key(key, app),
        }
    }
...
pub fn handle_input_in_subpage(key: KeyEvent, app: &Status, bus: &Bus) -> bool {
    ...
    if app.page.subpage_active {
        ...
        match app.page.subpage_tab_index {
            SubpageTab::Config => {
                return config_panel::handle_input(key, bus)
            }
            SubpageTab::Body => match app.page.app_mode {
                crate::protocol::status::AppMode::Modbus => {
                    return modbus_panel::handle_input(key, bus)
                }
                crate::protocol::status::AppMode::Mqtt => {
                    return mqtt_panel::handle_input(key, bus)
                }
            },
            SubpageTab::Log => {
                return log_panel::handle_input(key, bus)
            }
        }
    }
    false
...
pub fn render_panels(f: &mut Frame, area: Rect, app: &Status) {
    ...
    if app.page.subpage_active {
        ...
        match app.page.subpage_tab_index {
            SubpageTab::Config => {
                config_panel::render(f, area, app, None)
            }
            SubpageTab::Body => match app.page.app_mode {
                crate::protocol::status::AppMode::Modbus => {
                    modbus_panel::render(f, area, app)
                }
                crate::protocol::status::AppMode::Mqtt => {
                    mqtt_panel::render(f, area, app)
                }
            },
            SubpageTab::Log => log_panel::render(f, area, app),
        }
    } else {
