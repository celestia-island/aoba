    ...
    if let Some(form) = app.subpage_form.as_ref() {
        if form.registers.is_empty() {
            ...
            for (i, r) in form.registers.iter().enumerate() {
                ...
                line_spans.push(Span::raw(format!("#{}", i + 1)));
                line_spans.push(Span::raw(", "));

                // Layered state flags
                let this_selected = selected;
                let this_chosen = form.master_field_selected && form.master_edit_index == Some(i);
                let this_editing_field =
                    form.master_field_editing && form.master_edit_index == Some(i);
                let cur_field = if this_chosen {
                ...
                let base_style = if this_selected {
                    browse_style
                } else {
                    normal_style
                };
                ...
                // Slave ID field
                let id_active = matches!(cur_field, Some(MEF::Id)) && this_editing_field;
                let id_style = if id_active {
                    active_field_style
                } else if this_chosen && matches!(cur_field, Some(MEF::Id)) {
                    chosen_style
                } else {
                    base_style
                };
                if id_active {
                    ...
                    line_spans.push(Span::styled(format!("Slave ID = [{}]", content), id_style));
                } else {
                ...
                if matches!(cur_field, Some(MEF::Type)) && this_editing_field {
                    ...
                    line_spans.push(Span::styled(" >", base_style));
                } else if this_chosen && matches!(cur_field, Some(MEF::Type)) {
                    line_spans.push(Span::styled(
                        format!("[{}]", types[cur_type_idx]),
                        chosen_style,
                    ));
                } else {
                    line_spans.push(Span::styled(format!("{}", types[cur_type_idx]), base_style));
                }
                line_spans.push(Span::raw(", "));
                ...
                let start_style = if start_active {
                    active_field_style
                } else if this_chosen && matches!(cur_field, Some(MEF::Start)) {
                    chosen_style
                } else {
                    base_style
                };
                ...
                line_spans.push(Span::raw(" - "));
                let end_active = matches!(cur_field, Some(MEF::End)) && this_editing_field;
                let end_style = if end_active {
                    active_field_style
                } else if this_chosen && matches!(cur_field, Some(MEF::End)) {
                    chosen_style
                } else {
                    base_style
                };
                    ...
                    while addr < end_exclusive {
                        ...
                        for (ci, text) in cols.iter().enumerate() {
                            ...
                            let style = if let Some(MEF::Value(a)) = &cur_field {
                                if *a as usize == cell_addr && this_editing_field {
                                    active_field_style
                                } else if !this_editing_field && *a as usize == cell_addr {
                                    chosen_style
