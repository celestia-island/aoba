    ...
    while addr < end_exclusive {
        ...
        for col in 0..8 {
            ...
            if let Some(MEF::Value(a)) = &cur_field {
                if *a as usize == cur && editing {
                    let toggle_label = if r.mode == crate::protocol::status::RegisterMode::Coils {
                        // show placeholder underscore for editing boolean
                        if raw_val != 0 {
                            format!("[{}]", crate::i18n::lang().protocol.value_true)
                        } else {
                            format!("[{}]", crate::i18n::lang().protocol.value_false)
                        }
                    } else {
                        if form.master_input_buffer.is_empty() {
                            "_".to_string()
                        } else {
                            format!("[{}]", form.master_input_buffer)
                        }
                    };
                    spans.push(Span::styled(toggle_label, style));
                    continue;
                }
            }
