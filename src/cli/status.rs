...
use crate::tui::status::Status as GlobalStatus;
...
/// Manages periodic dumping of TUI global status to a JSON file for E2E testing.
pub struct TuiStatusDumper {
    task_handle: tokio::task::JoinHandle<()>,
    should_stop: Arc<AtomicBool>,
}

impl TuiStatusDumper {
    /// Create a new dumper and start the background task.
    pub fn new(status: Arc<Mutex<GlobalStatus>>) -> Self {
        let should_stop = Arc::new(AtomicBool::new(false));
        let should_stop_clone = should_stop.clone();

        let task_handle = tokio::spawn(async move {
            let status_path = PathBuf::from("/tmp/ci_tui_status.json");
            loop {
                if should_stop_clone.load(Ordering::Relaxed) {
                    break;
                }
                let status_lock = status.lock().await;
                let serializable_status = status_lock.to_serializable().await;
                drop(status_lock);

                match serializable_status.to_json() {
                    Ok(json) => {
                        if let Err(e) = fs::write(&status_path, json) {
                            log::error!("Failed to write TUI status file: {e}");
                        }
                    }
                    Err(e) => {
                        log::error!("Failed to serialize TUI status: {e}");
                    }
                }
                tokio::time::sleep(Duration::from_millis(500)).await;
            }
